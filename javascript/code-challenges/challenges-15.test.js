...
const screenForNames = (arr) => {
  // Solution code here...
  const properNames = /^(Mr\.|Mrs\.|Ms\.|Dr\.)\s[A-Za-z\s]+$/;
  return arr.filter((name) => properNames.test(name));
}
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  const thinLuke = parseInt(arr.find((character) => character.name === 'Luke Skywalker').mass);
  return arr
    .filter((character) => parseInt(character.mass) > thinLuke)
